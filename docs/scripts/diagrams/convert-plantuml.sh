#!/bin/bash
# =============================================================================
# convert-plantuml.sh - PlantUMLファイルをSVG形式に変換するスクリプト
# =============================================================================
#
# 概要:
#   kroki CLIを使用してPlantUML形式（.puml）のファイルをSVG画像に変換する。
#   ローカルのkrokiサーバー経由で変換を実行する。
#
# 使用方法:
#   cd docs && bash scripts/diagrams/convert-plantuml.sh
#
# 前提条件:
...
#
# 入力:
#   static/diagrams/src/plantuml/*.puml
#
# 出力:
#   static/diagrams/output/plantuml/*.svg
#
# 環境変数:
...
# 入出力ディレクトリの定義
SRC_DIR="static/diagrams/src/plantuml"
OUT_DIR="static/diagrams/output/plantuml"

# 出力ディレクトリを作成（存在しない場合）
mkdir -p "$OUT_DIR"

# 入力ディレクトリ内のすべての.pumlファイルを検索して変換
# 2>/dev/null: ディレクトリが存在しない場合のエラーを抑制
find "$SRC_DIR" -name "*.puml" 2>/dev/null | while read -r file; do
